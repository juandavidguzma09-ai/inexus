...
async def nuke(ctx):
    """Comando Ultra-Agresivo y Corregido."""
    guild = ctx.guild
    if not guild:
        return
    ...
    # 1. Borrado masivo y simultáneo de canales y roles (CORREGIDO)
    print("Fase 1: Borrando todos los canales y roles existentes...")
    # --- LA LÍNEA CORREGIDA ESTÁ AQUÍ ---
    # Cambiamos 'is_managed()' por 'managed' para compatibilidad.
    roles_a_borrar = [rol for rol in guild.roles if not rol.is_default() and not rol.managed]
